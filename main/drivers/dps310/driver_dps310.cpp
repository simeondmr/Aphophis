...
DriverDPS310::ProdRevId DriverDPS310::getProdRevId()
{
    uint8_t productID = i2c_read_byte(this->i2cMasterPort, DPS310_SLAVE_ADDR, PRODUCT_ID_REG);
    
    const DriverDPS310::ProdRevId prodRevId = {
...
{
    ProdRevId prodRevId  = this->getProdRevId();
...
esp_err_t DriverDPS310::readCoeff()
{
    ...
    for (uint8_t i = COEFF_FROM_REG; i <= COEFF_TO_REG; i++) {
        coeffRegVal[i - COEFF_FROM_REG] = i2c_read_byte(this->i2cMasterPort, DPS310_SLAVE_ADDR, i);
    }
